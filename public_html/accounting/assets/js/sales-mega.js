(function () {
  ...
  const LATEST_ENDPOINT = '../api/sales/mega_latest.php';
  const UPLOAD_ENDPOINT = '../api/sales/mega_upload.php';
  ...
  const titleEl = document.querySelector('[data-mega-title]');
  const navButtons = document.querySelectorAll('[data-mega-nav]');
  const tableBody = document.querySelector('[data-mega-rows]');
  const uploadButton = document.querySelector('[data-action="upload-mega"]');
  const downloadButton = document.querySelector('[data-action="download-mega"]');
  let uploadInput = document.querySelector('[data-mega-upload-input]');
  ...
  function bindEvents() {
    navButtons.forEach((button) => {
      button.addEventListener('click', () => {
        const dir = button.dataset.megaNav;
        if (dir === 'prev') {
          shiftMonth(-1);
        } else if (dir === 'next') {
          shiftMonth(1);
  ...
  function ensureUploadInput() {
    if (!(uploadInput instanceof HTMLInputElement)) {
      ...
      uploadInput.accept = '.xlsx,.xls,.csv,.pdf';
      uploadInput.setAttribute('data-mega-upload-input', 'dynamic');
  ...
  function updateTitle() {
    ...
    titleEl.textContent = `${rocYear}年${state.month}月兆豐銀行明細`;
  }
    ...
    fetch(`${LATEST_ENDPOINT}?${params.toString()}`, { credentials: 'same-origin' })
      ...
      .then((payload) => {
        ...
        if (payload.parse_error) {
          console.info('[mega] parse info:', payload.parse_error);
        }
      })
      .catch((error) => {
        console.error('[mega] load failed', error);
        if (tableBody) {
          tableBody.innerHTML = '<tr><td colspan="9" class="table-empty">載入失敗，請重新整理。</td></tr>';
        }
      });
  }
      ...
      .map((record, index) => {
        ...
        return `
          <tr data-index="${index}">
            ...
            <td class="text-left">${escapeHtml(record.note || '')}</td>
